...
import { type DotLottieStateMachine } from '@dotlottie/dotlottie-js';
    ...
    switch (currentFilePath) {
      case 'States':
        ...
        if (editorValue && currentFileName) {
          const newStateId = (editorValueParsed as DotLottieStateMachine).descriptor.id;

          addDotLottieStateMachine(editorValueParsed as DotLottieStateMachine, currentFileName);
          dispatch(
            setEditorFile({
              name: newStateId,
            }),
    ...
    async (title: string, file: File) => {
      ...
      switch (title) {
        case 'States':
          ...
          addDotLottieStateMachine(parsedContent as DotLottieStateMachine);
          break;
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="w-full bg-dark p-2 flex gap-2 justify-end">
        <Dropzone onDrop={onDrop} accept="lottie">
          {(state): JSX.Element => (
      ...
      <div className="flex grow border-t border-gray-600 flex-1 max-h-[calc(100vh-2.9rem)]">
        <PanelGroup autoSaveId="dotlottie-playground" direction="horizontal" className="h-full">
          ...
          <Panel>
            <Dropzone onDrop={onDrop} accept="lottie" noClick>
              {(state): JSX.Element => {
                return (
                  <div className="flex flex-col w-full h-full relative" {...state.getRootProps()}>
                    <input {...state.getInputProps()} />
                    {state.isDragActive && (
